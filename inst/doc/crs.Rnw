...
\item the degree of the spline for each dimension of the continuous
  ...
  cross-validation (holding the number of knots fixed at any user-set
  value)

\item the number of knots (`breakpoints') for each dimension of the
  continuous predictors can be manually specified or chosen by
  leave-one-out cross-validation (holding the degree of the spline
...
\item we allow the degree of the spline (alternatively, the number of
  knots) for each dimension of the continuous predictors to include
  zero and when the degree (alternatively, number of knots) is zero
