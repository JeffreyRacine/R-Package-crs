...
options for \code{tensor=} in \code{?crs}).
...
predictor from \code{degree=0} through \code{degree=degree.max}
...
z <- factor(z)
model <- crs(y~x+z,cv=TRUE,tensor="auto",kernel=TRUE)
summary(model)
@ 
...
z <- factor(z)
y <- dgp + rnorm(n,sd=1)
model <- crs(y~x1+x2+z,cv=TRUE,tensor="auto",degree.max=8)
summary(model)
@ 
...
model.res <- crs(y~x1+x2+z,degree=model$degree,tensor=model$tensor,include=0)
...
warned that this can be fragile when \code{tensor="enabled"}).
