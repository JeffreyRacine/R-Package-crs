...
Data-driven methods (leave-out-out cross-validation,
\citeauthor{STONE:1974} \citeyear{STONE:1974}, \citeauthor{STONE:1977}
\citeyear{STONE:1977}) can be used for selecting the spline degree,
number of segments/knots, and bandwidths. Details of the
...
\item alternatively, leave-one-out cross-validation can be used to
  select either the degree of the spline (holding the number of
...
\item when indicator basis functions are used instead of kernel
  ...
  be specified manually or chosen via leave-one-out cross-validation
...
\item we allow the degree of the spline for each continuous predictor
  ...
  automatically removed by cross-validation negating the need for
  pre-testing (\citeauthor{HALL_RACINE_LI:2004}
...
determined via leave-line-out cross-validation (see the options for
...
and bandwidth can be jointly determined via leave-one-out
...
The `generalized' cross-validation function is obtained by replacing
$h_{ii}$ with its average value denoted $\text{tr}(H)/n$ (see below
for the definition of $\text{tr}(H)$, \citep{CRAVEN_WAHBA:1979}).

The information-based function proposed by
...
where
\begin{equation*}
  \hat\sigma^2=\frac{1}{n}\sum_{i=1}^n\{Y_i-\hat g(X_i)\}^2
  =Y'(I-H)'(I-H)Y/n
\end{equation*}
with $\hat g(X_i)$ being a nonparametric estimator and $H$ being an
$n\times n$ weighting function (i.e., the matrix of kernel weights)
with its $(i,j)$th element given by $H_{ij} = K_{h,ij}/\sum_{l=1}^n
K_{h,il}$, $K_{h,ij} = \prod_{s=1}^q h_s^{-1}k((X_{is}-X_{js})/h_s)$,
and where $\text{tr}(H)$ means the trace of $H$.
