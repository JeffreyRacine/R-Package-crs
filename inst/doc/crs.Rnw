...
singularities. The tensor product basis is given by
\begin{equation*}
...
or tensor product bases can be automatically determined via any
\code{cv.func} method (see the options for \code{basis=} in
\code{?crs}).
...
by setting the option \code{cv="nomad"} (`nonsmooth mesh adaptive
direct search', see \citep{NOMAD}) or \code{cv="exhaustive"}
(exhaustive search). This then evaluates the model for each unique
combination of degree for each continuous predictor from
\code{degree=degree.min} through \code{degree=degree.max} (default 0
and 10, respectively) and from \code{segments=segments.min} through
\code{segments=segments.max} (default 1 and 10, respectively).

When conducting exhaustive search, for each unique combination of
degree, the bandwidths ($\in[0,1]$) are obtained via numerical
minimization (see \code{optim}) and restarting can be conducted via
\code{restarts=}. The model possessing the lowest criterion function
value is then selected as the final model.

When conducting \code{cv="nomad"} (nonsmooth mesh adaptive direct
search) the number of multiple starts can be controlled by
\code{nmulti=}.
...
\code{"data")}) support objects of this type. Figure \ref{crs plot}
