...
direct search) or \code{cv="exhaustive"} (exhaustive search). This
then evaluates the model for each unique combination of degree for
each continuous predictor from \code{degree=0} through
\code{degree=basis.maxdim} (default 5). For each unique combination of
...
z <- factor(z)
y <- dgp + rnorm(n,sd=1)
model <- crs(y~x1+x2+z,complexity="degree",segments=c(1,1),basis="auto",basis.maxdim=8)
summary(model)
@ 
