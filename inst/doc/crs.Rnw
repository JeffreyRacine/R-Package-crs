...
This package implements the approach described in
\citep{MA_RACINE_YANG:2010} when the option \code{kernel=TRUE} is
selected as described below. As well, this package implements related
methods that the author is currently developing.
...
or not to include the tensor product bases only, the additive model
bases only, or both the additive components and their tensor product
(`functional ANOVA' bases) can be automatically determined via
leave-line-out cross-validation (see the options for \code{basis=} in
\code{?crs}).
