%% $Id: spline_primer.Rnw,v 1.26 2011/04/26 20:20:54 jracine Exp jracine $
...
From \citeasnoun{DE_BOOR:2001} we knot that the derivatives of spline
...
regression setting we typically encounter multiple regressors which
can be continuous or categorical in nature, and traditional splines
are for continuous regressors. Below we briefly describe a
...
regressors). This method is implemented in the R package `crs'
(\citeasnoun{crs}).

\subsection{Multivariate knots, intervals, and spline bases}

In general we will have $q$ regressors,
...
vector of continuous regressors, and $\mathbf{Z}=\left( Z_{1},\ldots
  ,Z_{r}\right) ^{\mathrm{T}}$ is an $r$-dimensional vector of
categorical regressors. Letting $\mathbf{z} =\left( z_{s}\right)
...
In order to handle the presence of categorical regressors, we define
\begin{align}
...
categorical regressors. See \citeasnoun{MA_RACINE_YANG:2011} and
