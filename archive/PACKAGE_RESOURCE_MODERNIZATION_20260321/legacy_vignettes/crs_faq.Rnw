...
is 0.15-40. Kindly update your version should you not be using the
...
\subsection{I have never used \texttt{R} before. Can you direct me to some
  ...
  \texttt{R} website (\url{https://www.r-project.org}) and looking under
  Documentation/Manuals (\url{https://cran.r-project.org/manuals.html})
  ...
  (\url{https://cran.r-project.org/web/views/}) for
  ...
  \url{https://jeffreyracine.github.io/gallery/}.
...
See
\href{https://CRAN.R-project.org/view=Econometrics}{https://CRAN.R-project.org/view=Econometricsl} for further information.
...
\subsection{I noticed you have placed a new version of the \texttt{crs}
  ...
  (\url{https://CRAN.R-project.org/package=crs}), or go
...
opts <- list("MAX_BB_EVAL"=10000,
             ...
             "MIN_FRAME_SIZE"=paste("r",sqrt(.Machine$double.eps),sep=""),
             "DISPLAY_DEGREE"=3)
...
\section*{Changes from Version 0.15-39 to 0.15-40 [11-Feb-2026]}

\begin{itemize}

\item Major speedups in core CV/estimation helpers (kernel/spline paths),
  including reduced categorical overhead and improved caching.

\item Added warm-start optimization and formal S3 classes/methods for
  crsiv/crsivderiv (print/summary/plot), with enhanced reporting.

\item Updated CV penalty strategy (data-driven defaults and cv.func-aware
  scaling) to improve optimization robustness.

\item Extended test coverage and resolved check/doc issues (codoc/aliases,
  non-ASCII cleanups, and dependency declarations).

\item Improved npglpreg/MPI workflow handling and console-output controls
  for cleaner batch/interactive runs.
  
\end{itemize}  

\section*{Changes from Version 0.15-38 to 0.15-39 [28-Jan-2026]}

\begin{itemize}

\item Added ggplot2 to Suggests field in DESCRIPTION per CRAN requirements

\item Indexing issues with shrinkage addressed

\item Optimization of kernel(), prod.kernel(), integrate.trapezoidal(), NZD(), NZD\_pos(), matrix.combn(), splitFrame(), blank(), check.function(), dimBS()

\item Fixed index validation bug in crssigtest.R

\item Fixed ci=T deprecation in npglpreg.Rd

\item Fixed typos reported by users in man page
...
\item Increased default \verb+MAX_BB_EVAL+ from 500 to 10000 (makes a
  ...
  \verb+MIN_FRAME_SIZE+) to reflect actual machine precision (using R's
