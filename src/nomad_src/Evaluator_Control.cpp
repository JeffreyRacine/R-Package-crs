  ...
  if ( nY < 2 ) {
	...
	// at least two trial points are inside trust radius:
	  ...
	  if ( model.get_error_flag()     ||
		...
		model.eval_hf ( scaled_pt , h_min , h_norm , h_model , f_model );
		++nb_inside_radius;
	      }
	...
	model_stats.update_ES_stats ( nb_inside_radius , pts.size() );
      }
    }
  }
  ...
  const NOMAD::Double & h_model          )
{
  ...
  // 3. angle with last successful directions:
  if ( x->get_direction() ) {
